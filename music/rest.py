...
def transpose():
    ...
    if not 'song' in request.json or not 'key' in request.json:
        return jsonify({
            'response': 'Failed',
            'error': "The 'song' and 'key' must be supplied."})
...
def song_by_id(song_id):
    ...
    song.tempo = request.json.get('tempo') or None
...
def cache_files_in_database():
    """
    The folders and files are cached in the database, so that we have a
    persistent, local store that we can store additional metadata against a
    folder e.g. Youtube link to a song. The folders and files are in two tables
    with a parent-child relationship.

    Since Dropbox does not offer a reliable unique ID for a file, we can only
    identify the folders and files by name. If a folder or file is renamed,
    then we will lose the metadata stored against it. We will need to handle
    the removal of the old-named records ourselves.
    """
    ...
    for filename, value in files.iteritems():
        ...
        for file_meta in value:
            ...
            for field in [
                    'name', 'path', 'extension', 'size', 'mime_type', 'url']:
...
def admin_user_edit(user_id=None):
    ...
    if user_id:
        ...
        try:
            u = Person(
                request.form.get('email'), request.form.get('firstname'),
...
def song_tags(folder_id):
    if request.method == 'GET':
        ...
        return render_template(
            'snippet_song_tags.html', folder_id=folder_id,
            selected=folder.tags, unselected=unselected)
    elif request.method == 'POST':
        folder = Folder.query.get(folder_id)
